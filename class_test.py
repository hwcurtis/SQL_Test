...
# (Removed global instantiation and variables. Use the QueryTest class for all operations.)
...
class QueryTest:
    ...
    def multi_query_test(self, query):
        multi_result_raw = self.sql(query)
        if multi_result_raw is not None:
            multi_result = multi_result_raw.head()
        ...
        if student_31_check is not None and not student_31_check.empty:
            ...
            if (student_row['student_id'] == 31 and
                ...
                student_row['last_name'].strip() == 'Doe' and
                    int(student_row['grade_level']) == 12):
    ...
    def delete_query(self):
        ...
        self.conn.commit()
        print("Student 31 has been removed")
    ...
    def logical_operators_test(self, query):
        ...
        if result is not None:
            if list(result.columns) == expected_cols:
                if result['grade_level'].isin(['10th', '11th', '12th']).all() and result['grade'].isin(['A', 'B', 'C', 'D', 'F']).all():
    ...
    def groupby_query_test(self, query):
        ...
        provided_result = self.sql(query)
        expected_result = self.sql(internal_query)
        ...
        pd.testing.assert_frame_equal(
            provided_result.sort_values('grade_level').reset_index(drop=True),
